...
export default function CalendarioMensualHoras() {
    ...
    // Set de fechas (string "YYYY-MM-DD") que tienen bloqueo de día completo
    // Un día se considera "completamente bloqueado" cuando hay un bloqueo que cubre
    // todo el horario de atención (inicio antes de 09:00 y fin después de 21:59).
    const [diasCompletamenteBloqueados, setDiasCompletamenteBloqueados] = useState(new Set());

    // Carga bloqueos del profesional y calcula qué días están completamente bloqueados
    useEffect(() => {
        ...
        async function cargarDiasBloqueados() {
            try {
                ...
                data.forEach((bloqueo) => {
                    const horaIni  = (bloqueo.horaInicio ?? "").slice(0, 5);       // "HH:MM"
                    const horaFin  = (bloqueo.horaFinalizacion ?? "").slice(0, 5); // "HH:MM"
                    const fechaIni = (bloqueo.fechaInicio ?? "").slice(0, 10);     // "YYYY-MM-DD"
                    const fechaFin = (bloqueo.fechaFinalizacion ?? "").slice(0, 10);

                    // Bloqueo de día completo: cubre todo el horario de atención (9:00 - 22:00)
                    const esDiaCompleto = horaIni <= "09:00" && horaFin >= "22:00";
                    if (!esDiaCompleto || !fechaIni || !fechaFin) return;

                    // Expande el rango de fechas del bloqueo
                    const cursor = new Date(fechaIni + "T00:00:00");
                    const limite = new Date(fechaFin + "T00:00:00");
                    while (cursor <= limite) {
                        const ymd = formatDateToYMD(cursor);
                        diasBloqueados.add(ymd);
                        cursor.setDate(cursor.getDate() + 1);
                    }
                });

                setDiasCompletamenteBloqueados(diasBloqueados);
            } catch (err) {
            ...
            <div className="mx-auto w-full max-w-3xl">
                <header className="mb-6 flex flex-col items-center gap-2 text-center">
                    ...
                    <p className="max-w-md text-sm leading-6 text-slate-500">
                        Reserva tu hora en línea en segundos. Selecciona fecha y un bloque horario disponible.
                    </p>
                </header>
                    ...
                    <div className="mt-4 grid grid-cols-7 gap-2 rounded-xl bg-slate-900/[0.02] p-2 ring-1 ring-slate-900/5">
                        ...
                        {dias.map((dia, i) =>
                            dia ? (() => {
                                ...
                                const isFullyBlocked = diasCompletamenteBloqueados.has(formatDateToYMD(dia));
                                const isDisabled = isPastDay || isSunday || isFullyBlocked;
                                ...
                                return (
                                    <button
                                        ...
                                        type="button"
                                        disabled={isDisabled}
                                        title={isFullyBlocked ? "Día no disponible" : undefined}
                                        onClick={() => {
                                            if (isDisabled) return;
                                            seleccionarFecha(dia);
                                        }}
                                        className={
                                            "h-10 flex items-center justify-center rounded-md text-sm font-medium transition focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-1 relative " +
                                            (isPastDay || isSunday
                                                ? "cursor-not-allowed border border-slate-200/70 bg-white/60 text-slate-300 shadow-sm opacity-50"
                                                : isFullyBlocked
                                    ...
                                    >
                                        {dia.getDate()}
                                        {/* Barra roja sutil para días completamente bloqueados */}
                                        {isFullyBlocked && (
                                            <span className="absolute inset-x-1 bottom-1 h-0.5 rounded-full bg-slate-300" />
                                        )}
                                    </button>
                                );
                            })() : (
